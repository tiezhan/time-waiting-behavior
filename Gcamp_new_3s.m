...
wave_ntrial_df_f= (wave_ntrial - base_mean_ntrial) ./ (base_mean_ntrial - wave_raw(10));
...
id = find(wave_ntrial_df_f(:) <= -1); %% find outliers
wave_ntrial_df_f(id,:) = [];%% delete outliers
id = find(wave_ntrial_df_f(:,100) >= wave_ntrial_df_f(:,400));  %% select none-ramp trials
...
id = find(wave_ntrial_df_f(:,250) >= wave_ntrial_df_f(:,400));
wave_ntrial_df_f(id,:) = []; 
...
zramp = sortrows(ztest, 400, 'ascend')
...
zNramp = sortrows(ztest, 400, 'ascend')
...
zall = sortrows(ztest, 400, 'ascend')
...
%% save data
pathname = 'D:\lutiezhan\dataALL\fiber_recording\DAT-cre-vta\3s\3s-DAT-GCaMP6recording-VTA-withHM-Duration\DAY1\'; %% choose file location
filename = 'mice41115.mat'; 
