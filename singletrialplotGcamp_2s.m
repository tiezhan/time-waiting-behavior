...
%%
% barline(tRise, [0.5 1.2]);
% dat = sub3.Signal1;
% twin = [-3 8];
% [Alg_sXtrail, ttick] = BF_AlignFsWave2Tg(Fs, dat, tRise, twin(1), twin(2), true);
...
eee = []
for i = 1:length(et)
    eee(i,:) = wave_raw(et(i)*100:(et(i)+ 5.666)*100)
end
...
wave_df_f = []
for i = 1:size(eee,1)
    wave_df_f(i,:) = (eee(i,:) - base_mean_ntrial(i)) ./ (base_mean_ntrial(i) - 2.8)
end
